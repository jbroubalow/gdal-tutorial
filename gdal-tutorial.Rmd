...
The Geospatial Data Abstraction Library (GDAL) is a computer software library for reading and writing raster and vector geospatial data formats. What we're going to do is use GDAL to map census data after downloading and simplifying our spatial data. Both the census data and spatial data will come from the NHGIS, but there are two problems with the spatial data that don't make them immediately useable to us in R: 1) They are too big and 2) They are in the wrong projection.
...
The next step is to simplify the shapefile within the spatial data so that it is easier to manage and read into RStudio. We will do this step in our web browsers using <http://www.mapshaper.org/> and RStudio.

1. Use the home screen to upload your shapefile to Map Shaper. Click "Upload" to do so.
...
5. Upload the `.zip` folder containing your new shapefile. Also upload the the folder containing the census data.

6. Read in the shapefile and the census data (Note that I've also renamed the NGHIS code "A7L001" for the sake of clarity.
